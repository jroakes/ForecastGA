...
class TestQuerying(base.TestCase):
    def test_raw(self):
        ...
        b = (
            self.query.set(metrics=["ga:pageviews"])
    ...
    def test_step(self):
        ...
        q = (
            self.query.metrics("pageviews")
            .range("2014-07-01", "2014-07-05")
            .interval("day")
            .step(2)
        )
        report = q.get()
    ...
    def test_sort(self):
        ...
        sorted_report = q.sort("pageviews").get()
        inverse_sorted_report = q.sort(-self.profile.core.columns["pageviews"]).get()
    ...
    def test_filter_additive(self):
        q = (
            self.query.metrics("pageviews")
            .filter(medium=["cpc", "cpm"])
            .filter(usertype__neq="Returning User")
            .build()
        )
        self.assertEqual(
            q["filters"], "ga:medium==cpc,ga:medium==cpm;ga:userType!=Returning User"
        )
