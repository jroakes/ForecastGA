...
def from_args(
    scope, metrics, start, stop, days, limit, dimensions, filter, segment, **description
):
...
@click.option("--start", help="Start date in ISO format, e.g. 2016-01-01.")
@click.option("--stop")
@click.option(
    "--days",
...
@click.option("--limit", help="Return only the first <n> or <start>,<n> results.")
@click.option(
    "--sort", help="Sort by a metric; prefix with - to sort from high to low."
)
...
@click.option("--with-metadata", is_flag=True)
