...
class TestReporting(base.TestCase):
    def test_tuples(self):
        ...
        report = (
            self.query.metrics("pageviews", "sessions")
            .dimensions("pagepath")
            .range("yesterday")
            .get()
        )
    ...
    def test_columnwise(self):
        ...
        report = (
            self.query.metrics("pageviews").dimensions("pagepath").daily(days=-10).get()
        )
    ...
    def test_serialization(self):
        """ serialized rows """
        serialized = (
            self.query.metrics("pageviews", "sessions").daily(days=-10).serialize()
        )
