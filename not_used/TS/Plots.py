...
def add_patched_legend(ax, names):
    ...
    for name in names:
        ...
        patched_name = (
            patched_name[1:] if (patched_name.startswith("_")) else patched_name
        )
...
def decomp_plot(
    df, time, signal, estimator, residue, name=None, format="png", max_length=1000
):
    assert df.shape[0] > 0
    assert df.shape[1] > 0
...
def decomp_plot_as_png_base64(
    df, time, signal, estimator, residue, name=None, max_length=1000
):
    assert df.shape[0] > 0
    assert df.shape[1] > 0
    assert time in df.columns
...
def prediction_interval_plot(
    df, time, signal, estimator, lower, upper, name=None, format="png", max_length=1000
):
    assert df.shape[0] > 0
    assert df.shape[1] > 0
    assert time in df.columns
    ...
    axs.fill_between(x.values, df1[lower], df1[upper], color=SHADED_COLOR, alpha=0.2)
...
def prediction_interval_plot_as_png_base64(
    df, time, signal, estimator, lower, upper, name=None, max_length=1000
):
    assert df.shape[0] > 0
    assert df.shape[1] > 0
    assert time in df.columns
    ...
    if type1.kind == "M":
        x = x.apply(lambda t: t.date())
    axs.fill_between(x.values, df1[lower], df1[upper], color=SHADED_COLOR, alpha=0.5)
