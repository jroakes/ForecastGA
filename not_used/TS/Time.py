...
class cTimeInfo:
    ...
    def transformDataset(self, df):
        ...
        df[self.mNormalizedTimeColumn] = self.compute_normalize_date_column(
            df[self.mTime]
        )
        # print(df.tail());
        return df
    ...
    def adaptTimeDeltaToTimeResolution(self):
        ...
        if cTimeInfo.sRES_SECOND == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                seconds=round(self.mTimeDelta / np.timedelta64(1, "s"))
            )
            return
        if cTimeInfo.sRES_MINUTE == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                minutes=round(self.mTimeDelta / np.timedelta64(1, "m"))
            )
            return
        if cTimeInfo.sRES_HOUR == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                hours=round(self.mTimeDelta / np.timedelta64(1, "h"))
            )
            return
        if cTimeInfo.sRES_DAY == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                days=round(self.mTimeDelta / np.timedelta64(1, "D"))
            )
            return
        if cTimeInfo.sRES_MONTH == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                months=round(self.mTimeDelta // np.timedelta64(30, "D"))
            )
            return
        if cTimeInfo.sRES_YEAR == self.mResolution:
            self.mTimeDelta = pd.DateOffset(
                months=round(self.mTimeDelta // np.timedelta64(365, "D"))
            )
            return
        pass
    ...
    def estimate(self):
        ...
        ] = self.compute_normalize_date_column(self.mSignalFrame[self.mTime])
        self.dump()
