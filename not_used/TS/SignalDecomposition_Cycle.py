...
class cZeroCycle(cAbstractCycle):
    ...
    def fit(self):
        ...
        self.mCycleFrame[self.getCycleName()] = np.zeros_like(
            self.mTrendFrame[self.mTrend_residue_name]
        )
        self.mCycleFrame[self.getCycleResidueName()] = self.mCycleFrame[
            self.mTrend_residue_name
        ]
...
class cSeasonalPeriodic(cAbstractCycle):
    ...
    def fit(self):
        ...
        lGroupBy = lCycleFrameEstim.groupby(by=[lName], sort=False)[
            self.mTrend_residue_name
        ].mean()
        ...
        self.mCycleFrame[lName + "_enc"] = self.mCycleFrame[lName].apply(
            lambda x: self.mEncodedValueDict.get(x, self.mDefaultValue)
        )
        self.mCycleFrame[lName + "_enc"].fillna(lTrendMeanEstim, inplace=True)
        self.mCycleFrame[self.getCycleResidueName()] = (
            self.mCycleFrame[self.mTrend_residue_name]
...
class cBestCycleForTrend(cAbstractCycle):
    ...
    def computeBestCycle(self):
        ...
        if self.mCyclePerfDict:
            for k in sorted(self.mCyclePerfDict.keys()):
                ...
                if (lBestCriterion is None) or (
                    self.mCyclePerfDict[k] < lBestCriterion
                ):
                    lBestCycleIdx = k
                    lBestCriterion = self.mCyclePerfDict[k]

            if self.mOptions.mCycle_Criterion_Threshold is None or (
                self.mCyclePerfDict[lBestCycleIdx]
                < self.mOptions.mCycle_Criterion_Threshold
            ):
    ...
    def generate_cycles(self):
        ...
        for i in lCycleLengths:
            if (i > 1) and (i <= lMaxRobustCycle):
                ...
                lCycleFrame[name_i] = (
                    self.mCycleFrame[self.mTimeInfo.mRowNumberColumn] % i
                )
                lCycleFrameEstim = self.mSplit.getEstimPart(lCycleFrame)
                lGroupBy = lCycleFrameEstim.groupby(by=[name_i], sort=False)[
                    self.mTrend_residue_name
                ].mean()
    ...
    def transformDataset(self, df):
        if self.mBestCycleLength is not None:
            lValueCol = df[self.mTimeInfo.mRowNumberColumn].apply(
                lambda x: x % self.mBestCycleLength
            )
            ...
            df[self.getCycleName()] = lValueCol.apply(
                lambda x: lDict.get(x, self.mDefaultValue)
            )
        else:
        ...
        for trend in self.mTrendList:
            ...
            for cycle in self.mCycleList[trend]:
                ...
                if self.mOptions.mDebugProfile:
                    logger = tsutil.get_pyaf_logger()
                    logger.info(
                        "CYCLE_TRAINING_TIME_IN_SECONDS '"
                        + cycle.mOutName
                        + "' "
                        + str(lTrainingTime)
                    )
        ...
        for trend in self.mTrendList:
            ...
            for cycle in self.mCycleList[trend]:
                if isinstance(cycle, cSeasonalPeriodic):
                    cycle.computePerf()
                    lPerfs[cycle.mOutName] = cycle.mCycleForecastPerf.getCriterionValue(
            ...
            for (k, cycle) in enumerate(lSeasonals):
                ...
                if (lBestCriterion is None) or (
                    lCriterionValue < (1.05 * lBestCriterion)
                ):
                    lBestSeasonal = cycle
            ...
            logger.debug(
                "CYCLE_TRAINING_FILTER_SEASONALS "
                + trend.mOutName
                + " "
