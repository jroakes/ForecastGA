...
class cSignalHierarchy:
    ...
    def to_json(self):
        ...
        for level in sorted(self.mModels.keys()):
            for signal in sorted(self.mModels[level].keys()):
                ...
                lDict["Models"][
                    signal
                ] = lEngine.mSignalDecomposition.mBestModel.to_json()
        return lDict
    ...
    def checkData(self, df):
        ...
        if self.mExogenousData is not None:
            ...
            if self.mDateColumn not in lExogenousDataFrame.columns:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_HIERARCHY_TIME_COLUMN_NOT_FOUND_IN_EXOGENOUS "
                    + str(self.mDateColumn)
                )
            for exog in lExogenousVariables:
                if exog not in lExogenousDataFrame.columns:
                    raise tsutil.PyAF_Error(
                        "PYAF_ERROR_HIERARCHY_EXOGENOUS_VARIABLE_NOT_FOUND " + str(exog)
                    )

            type3 = np.dtype(lExogenousDataFrame[self.mDateColumn])
            if type1 != type3:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_HIERARCHY_INCOMPATIBLE_TIME_COLUMN_TYPE_IN_EXOGENOUS '"
                    + str(self.mDateColumn)
                    + "' '"
                    + str(type1)
                    + "' '"
    ...
    def create_all_levels_models(self, iAllLevelsDataset, H, iDateColumn):
        ...
        for level in sorted(self.mStructure.keys()):
            ...
            for signal in sorted(self.mStructure[level].keys()):
                logger.info(
                    "TRAINING_MODEL_LEVEL_SIGNAL " + str(level) + " " + str(signal)
                )
        ...
        for level in sorted(self.mModels.keys()):
            for signal in sorted(self.mModels[level].keys()):
                ...
                lEngine.standardPlots(
                    name + "_Hierarchy_Level_Signal_" + str(level) + "_" + str(signal)
                )
    ...
    def forecastAllModels(self, iAllLevelsDataset, H, iDateColumn):
        ...
        for level in sorted(self.mModels.keys()):
            for signal in sorted(self.mModels[level].keys()):
                ...
                lForecast_DF[str(signal) + "_Forecast"] = dfapp_out[
    ...
    def computePerfOnCombinedForecasts(self, iForecast_DF):
        ...
        for level in sorted(self.mStructure.keys()):
            ...
            logger.info(
                "MODEL_LEVEL " + str((level, sorted(list(self.mModels[level].keys()))))
            )
            for signal in sorted(self.mStructure[level].keys()):
                lEngine = self.mModels[level][signal]
                ...
                for iPrefix in lPrefixes:
                    lPerfFit_Combined = lEngine.computePerf(
                        lFrameFit[signal],
                        lFrameFit[str(signal) + "_" + iPrefix + "_Forecast"],
                        signal,
                    )
                    lPerfValid_Combined = lEngine.computePerf(
                        lFrameValid[signal],
                        lFrameValid[str(signal) + "_" + iPrefix + "_Forecast"],
                        signal,
                    )
        ...
        for (sig, perf) in sorted(lPerfs.items()):
            logger.info(
                "REPORT_COMBINED_FORECASTS_FIT_PERF "
                + str((sig, perf[0].mL2, perf[0].mMAPE, perf[2].mL2, perf[2].mMAPE))
            )
            logger.info(
                "REPORT_COMBINED_FORECASTS_VALID_PERF "
                + str((sig, perf[1].mL2, perf[1].mMAPE, perf[3].mL2, perf[3].mMAPE))
            )
        return lPerfs
    ...
    def internal_forecast(self, iInputDS, iHorizon):
        ...
        lForecast_DF = self.forecastAllModels(
