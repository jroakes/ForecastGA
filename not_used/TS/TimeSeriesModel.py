...
class cTimeSeriesModel:
    ...
    def getComplexity(self):
        lComplexity = (
            32 * self.mTransformation.mComplexity
            + 16 * self.mTrend.mComplexity
            + 4 * self.mCycle.mComplexity
            + 1 * self.mAR.mComplexity
        )
        return lComplexity
    ...
    def forecastOneStepAhead(self, df, horizon_index=1, perf_mode=False):
        ...
        if not perf_mode:
            ...
            df2[lPrefix + "Cycle_residue"] = (
                df2[lPrefix + "Trend_residue"] - lCycleColumn
            )
    ...
    def addPredictionIntervals(self, iInputDS, iForecastFrame, iHorizon):
        ...
        for h in range(0, lHorizon):
            ...
            iForecastFrame.loc[N + h, lLowerBoundName] = (
                iForecastFrame.loc[N + h, lForecastColumn] - lWidth
            )
            iForecastFrame.loc[N + h, lUpperBoundName] = (
                iForecastFrame.loc[N + h, lForecastColumn] + lWidth
            )
    ...
    def plotForecasts(self, df):
        ...
        tsplot.decomp_plot(
            df, lTime, lPrefix + "Signal", lPrefix + "Forecast", lPrefix + "Residue"
        )

    def to_json(self):
        dict1 = {}
        d1 = {
            "Time": self.mTimeInfo.to_json(),
        ...
        dict1["Model"] = d2
        d3 = {
            "MAPE": str(self.mForecastPerf.mMAPE),
    ...
    def plotResidues(self, name=None, format="png"):
        ...
        if name is not None:
            ...
            tsplot.decomp_plot(
                df, lTime, self.mSignal, lPrefix + "Trend", lPrefix + "Trend_residue"
            )
            tsplot.decomp_plot(
                df,
                lTime,
    ...
    def getPlotsAsDict(self):
        ...
        lTime = self.mTimeInfo.mTime
        lOutput.set_index(lTime, inplace=True, drop=False)
        lDict["Prediction_Intervals"] = tsplot.prediction_interval_plot_as_png_base64(
            lOutput,
            lTime,
            self.mOriginalSignal,
            lForecastColumn,
