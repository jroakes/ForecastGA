...
class cAbstract_Scikit_Model(tsar.cAbstractAR):
    ...
    def fit(self):
        ...
        self.mARFrame[self.mOutName + "_residue"] = (
            self.mARFrame[series] - self.mARFrame[self.mOutName]
        )
    ...
    def transformDataset(self, df, horizon_index=1):
        ...
        inputs_after_feat_selection = (
            self.mFeatureSelector.transform(inputs) if self.mFeatureSelector else inputs
        )
        if self.mScikitModel is not None:
...
class cXGBoost_Model(cAbstract_Scikit_Model):
    ...
    def get_default_xgb_options(self):
        lXGBOptions = dict(
            n_estimators=10, nthread=1, min_child_weight=10, max_depth=3, seed=1960
        )
        return lXGBOptions
