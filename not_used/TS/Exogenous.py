...
class cExogenousInfo:
    ...
    def createEncodedExogenous(self):
        ...
        self.mEncodedExogenousDataFrame[self.mDateVariable] = self.mExogenousDataFrame[
            self.mDateVariable
        ]
                ...
                if lList is not None:
                    for lCat in lList:
                        ...
                        self.mEncodedExogenousDataFrame[lDummyName] = np.where(
                            self.mExogenousDataFrame[exog] == lCat,
                            np.int8(1),
                            np.int8(0),
                        )
                    ...
                    self.mEncodedExogenousDataFrame[exog] = np.float32(
                        (self.mExogenousDataFrame[exog] - lExogStats[0]) / lExogStats[1]
                    )
                    self.mEncodedExogenousDataFrame[exog].fillna(
                        np.float32(0.0), inplace=True
                    )
