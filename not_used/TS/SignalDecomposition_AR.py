...
class cAutoRegressiveEstimator:
    ...
    def addLagsForTraining(self, df, cycle_residue, iNeedExogenous=False):
        ...
        for autoreg in self.mARList[cycle_residue]:
            ...
            if autoreg.mExogenousInfo is not None:
                assert (P + P * len(autoreg.mExogenousInfo.mEncodedExogenous)) >= len(
                    autoreg.mInputNames
                )
            else:
    ...
    def estimate_ar_models_for_cycle(self, cycle_residue):
        ...
        self.mARFrame = pd.DataFrame()
        self.mTimeInfo.addVars(self.mARFrame)
        self.mCycleFrame[cycle_residue] = self.mCycleFrame[cycle_residue].astype(
            np.float64
        )
        self.mARFrame[cycle_residue] = self.mCycleFrame[cycle_residue].astype(
            np.float64
        )
        ...
        for autoreg in self.mARList[cycle_residue]:
            ...
            if self.mOptions.mDebugProfile:
                logger.info(
                    "AR_MODEL_START_TRAINING_TIME '"
                    + cycle_residue
                    + "' "
                    + str(self.mCycleFrame.shape[0])
                    + " "
                    + str(len(autoreg.mInputNames))
                    + " "
                    + str(start_time)
                )
            ...
            if self.mOptions.mDebugProfile:
                logger.info(
                    "AR_MODEL_TRAINING_TIME_IN_SECONDS '"
                    + autoreg.mOutName
                    + "' "
                    + str(self.mCycleFrame.shape[0])
                    + " "
    ...
    def estimate(self):
        ...
        for trend in self.mTrendList:
            for cycle in self.mCycleList[trend]:
                ...
                if (self.mCycleFrame[cycle_residue].shape[0] > 12) and (
                    ...
                    if self.mOptions.mActiveAutoRegressions["AR"]:
                        ...
                        self.mARList[cycle_residue] = self.mARList[cycle_residue] + [
                            lAR
                        ]
                    if self.mOptions.mActiveAutoRegressions["ARX"] and (
                        self.mExogenousInfo is not None
                    ):
                        lARX = tsscikit.cAutoRegressiveModel(
                            cycle_residue, lLags, self.mExogenousInfo
                        )
                        self.mARList[cycle_residue] = self.mARList[cycle_residue] + [
                            lARX
                        ]
                    ...
                    if self.mOptions.mActiveAutoRegressions["LSTM"]:
                        if self.mOptions.canBuildKerasModel("LSTM"):
                            lLSTM = tskeras.cLSTM_Model(
                                cycle_residue, lLags, self.mExogenousInfo
                            )
                            self.mARList[cycle_residue] = self.mARList[
                                cycle_residue
                            ] + [lLSTM]
                        else:
                    ...
                    if self.mOptions.mActiveAutoRegressions["SVRX"] and (
                        self.mExogenousInfo is not None
                    ):
                        lSVRX = tsscikit.cSVR_Model(
                            cycle_residue, lLags, self.mExogenousInfo
                        )
                        self.mARList[cycle_residue] = self.mARList[cycle_residue] + [
                            lSVRX
                    ...
                    if self.mOptions.mActiveAutoRegressions["XGBX"] and (
                        self.mExogenousInfo is not None
                    ):
                        lXGBX = tsscikit.cXGBoost_Model(
                            cycle_residue, lLags, self.mExogenousInfo
                        )
                        self.mARList[cycle_residue] = self.mARList[cycle_residue] + [
                            lXGBX
