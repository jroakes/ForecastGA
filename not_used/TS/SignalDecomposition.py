...
class cSignalDecompositionOneTransform:
    ...
    def setParams(
        self, iInputDS, iTime, iSignal, iHorizon, iTransformation, iExogenousData=None
    ):
    ...
    def updatePerfsForAllModels(self, iModels):
        ...
        for (name, model) in iModels.items():
            ...
            self.mPerfsByModel[model.mOutName] = [
                model,
                lComplexity,
                lFitPerf,
                lForecastPerf,
    ...
    def collectPerformanceIndices(self):
        ...
        self.mPerfDetails = pd.DataFrame(
            rows_list,
            columns=(
                "Model",
                "Complexity",
    ...
    def train(self, iInputDS, iTime, iSignal, iHorizon, iTransformation):
        ...
        self.setParams(
            iInputDS, iTime, iSignal, iHorizon, iTransformation, self.mExogenousData
        )
        ...
        if self.mExogenousInfo is not None:
            ...
            if self.mOptions.mDebugProfile:
                logger.info(
                    "EXOGENOUS_ENCODING_TIME_IN_SECONDS "
                    + str(self.mSignal)
                    + " "
                    + str(time.time() - exog_start_time)
                )
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                "TREND_TIME_IN_SECONDS "
                + str(self.mSignal)
                + " "
                + str(time.time() - trend_start_time)
            )
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                ...
                + str(self.mSignal)
                + " "
                + str(str(time.time() - cycle_start_time))
            )
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                "AUTOREG_TIME_IN_SECONDS "
                + str(self.mSignal)
                + " "
                + str(str(time.time() - ar_start_time))
            )
        # forecast perfs
            ...
            for cycle in lAREstimator.mCycleList[trend]:
                ...
                for autoreg in lAREstimator.mARList[cycle_residue]:
                    lModel = tsmodel.cTimeSeriesModel(
                        self.mTransformation, trend, cycle, autoreg
                    )
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                "PERF_TIME_IN_SECONDS "
                + str(self.mSignal)
                + " "
                + str(len(lModels))
                + " "
                + str(str(time.time() - perf_start_time))
            )
...
def run_transform_thread(arg):
    ...
    arg.mSigDec.train(
        arg.mInputDS, arg.mTime, arg.mSignal, arg.mHorizon, arg.mTransformation
    )
    return arg
...
class cSignalDecompositionTrainer:
    ...
    def defineTransformations(self, df, iTime, iSignal):
        ...
        if self.mOptions.mActiveTransformations["None"]:
            self.validateTransformation(
                tstransf.cSignalTransform_None(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["Difference"]:
            self.validateTransformation(
                tstransf.cSignalTransform_Differencing(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["RelativeDifference"]:
            self.validateTransformation(
                tstransf.cSignalTransform_RelativeDifferencing(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["Integration"]:
            self.validateTransformation(
                tstransf.cSignalTransform_Accumulate(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["BoxCox"]:
            for i in self.mOptions.mBoxCoxOrders:
                self.validateTransformation(
                    tstransf.cSignalTransform_BoxCox(i), df, iTime, iSignal
                )

        if self.mOptions.mActiveTransformations["Quantization"]:
            for q in self.mOptions.mQuantiles:
                self.validateTransformation(
                    tstransf.cSignalTransform_Quantize(q), df, iTime, iSignal
                )

        if self.mOptions.mActiveTransformations["Logit"]:
            self.validateTransformation(
                tstransf.cSignalTransform_Logit(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["Fisher"]:
            self.validateTransformation(
                tstransf.cSignalTransform_Fisher(), df, iTime, iSignal
            )

        if self.mOptions.mActiveTransformations["Anscombe"]:
            self.validateTransformation(
                tstransf.cSignalTransform_Anscombe(), df, iTime, iSignal
            )
    ...
    def collectPerformanceIndices_ModelSelection(self):
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                "MODEL_SELECTION_TIME_IN_SECONDS "
                + str(self.mBestModel.mSignal)
                + " "
                + str(time.time() - modelsel_start_time)
            )
    ...
    def collectPerformanceIndices(self):
        ...
        for transform1 in self.mTransformList:
            ...
            for (model, value) in sorted(sigdec.mPerfsByModel.items()):
                ...
                rows_list.append(row)
                if self.mOptions.mDebugPerformance:
                    lIndicatorValue = lForecastPerf.getCriterionValue(
                        self.mOptions.mModelSelection_Criterion
                    )
                    logger.info(
                        "collectPerformanceIndices : "
...
class cSignalDecompositionTrainer_CrossValidation:
    ...
    def train(self, iInputDS, iTime, iSignal, iHorizon):
        ...
        for lSplit in self.mSplits:
            ...
            logger.info(
                "CROSS_VALIDATION_TRAINING_SIGNAL_SPLIT '"
                + iSignal
                + "' "
                + str(lSplit)
            )
            ...
            logger.info(
                "CROSS_VALIDATION_TRAINING_SPLIT_TIME_IN_SECONDS '"
                + iSignal
                + "' "
                + str(lSplit)
                + " "
                + str(time.time() - split_start_time)
            )
        self.perform_model_selection()
        logger.info(
            "CROSS_VALIDATION_TRAINING_TIME_IN_SECONDS "
            + str(self.mBestModel.mSignal)
            + " "
            + str(time.time() - cross_val_start_time)
        )
    ...
    def perform_model_selection(self):
        ...
        lPerfByCategory_df.sort_values(
            by=[lIndicator, "Category"], ascending=[True, True], inplace=True
        )
...
class cSignalDecomposition:
    ...
    def checkData(self, iInputDS, iTime, iSignal, iHorizon, iExogenousData):
        ...
        if iSignal not in iInputDS.columns:
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_SIGNAL_COLUMN_NOT_FOUND " + str(iSignal)
            )
    ...
    def train(self, iInputDS, iTime, iSignal, iHorizon, iExogenousData=None):
        ...
        if self.mOptions.mDebugProfile:
            logger.info(
                "PREDICTION_INTERVAL_TIME_IN_SECONDS "
                + str(iSignal)
                + " "
                + str(time.time() - pred_interval_start_time)
            )

        end_time = time.time()
        self.mTrainingTime = end_time - start_time
        logger.info(
            "END_TRAINING_TIME_IN_SECONDS '"
            + str(iSignal)
            + "' "
    ...
    def forecast(self, iInputDS, iHorizon):
        ...
        lInputDS[
            self.mBestModel.mOriginalSignal
        ] = lMissingImputer.interpolate_signal_if_needed(
            iInputDS, self.mBestModel.mOriginalSignal
        )
        lInputDS[self.mBestModel.mTime] = lMissingImputer.interpolate_time_if_needed(
            iInputDS, self.mBestModel.mTime
        )
